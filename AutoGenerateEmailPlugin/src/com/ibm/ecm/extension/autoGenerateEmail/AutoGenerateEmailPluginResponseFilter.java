 ...
 */
public class AutoGenerateEmailPluginResponseFilter extends PluginResponseFilter {
	...
	public String[] getFilteredServices() {
		return new String[]{"/getDesktop", "/p8/openItem", "/p8/openFolder"};
	}

	/**
	 * Filters the response from the "getDesktop" service, inserting the <code>AutoGenerateEmail</code> plug-in setting.
	 *
	 * @param serverType   A <code>String</code> that indicates the type of server that
	 ...
	 *                     content server APIs.
	 * @param request      An <code>HttpServletRequest</code> object that provides the
	 *                     request. The service can access the invocation parameters from
	 *                     the request.
	 * @param jsonResponse The <code>JSONObject</code> object that is generated by the
	...
	public void filter(String serverType, PluginServiceCallbacks callbacks, HttpServletRequest request, JSONObject jsonResponse) throws Exception {
		ResourceBundle centralizedMessages = ResourceBundle.getBundle("com.ibm.ecm.extension.autoGenerateEmail.nls.Messages");
		...
		if (configStr != null && configStr.length() > 0 && jsonResponse != null) {
			...
			try {
				...
				if (config != null) {
					...
					JSONObject autoGenerateEmailPlugin = new JSONObject();
					autoGenerateEmailPlugin.put("emailList", config.get("emailList"));
					pluginSettings.put(AutoGenerateEmailPlugin.PLUGIN_ID, autoGenerateEmailPlugin);
				}
			} catch (Exception e) {
				callbacks.getLogger().logError(this, "filter", centralizedMessages.getString("error.Text.summary"), e);
			}
